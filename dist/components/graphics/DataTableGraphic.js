...
export const DataTableGraphic = ({ apiUrl, title }) => {
    ...
    return (<>
            <Modal type={"primary"} action="Editar" title="Editar fila" id="insertModal" properties={dataProperties} data={editData} isOpen={showEditModal} onClose={handleCloseModal}/>
            <Modal type={"danger"} action="Eliminar" title="Eliminar fila" id="deleteModal" data={editData} isOpen={showDeleteModal} onClose={handleCloseModal}/>
            ...
            <div className="card my-4">
                ...
                <div className="card-body table-responsive">
                    ...
                    {data !== null && dataProperties.length > 0 && (<table id="datatablesSimple" className='table table-bordered'>
                            <thead>
                                <tr>
                                    {dataProperties.map(prop => (<th key={prop}>{prop}</th>))}
                                    <th>Editar</th>
                                    <th>Eliminar</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.map((item, index) => (<tr key={item.id} name={item.id}>
                                        {dataProperties.map(prop => (<td key={prop} name={prop}>
