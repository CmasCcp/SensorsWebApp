...
export const Pagination = ({ totalPages = 1, currentPage, nextPage, prevPage, goToPage }) => {
    ...
    return (<div className='col-lg-10 mx-auto max-width-100 col-md-12 col-sm-12 row justify-content-around p-0 mb-4 m-0 flex-nowrap'>
            ...
            return (<div key={pageNumber} className={`button ${(totalPages > numero_de_botones && (pageNumber > currentPage + numero_de_botones) || (pageNumber < currentPage - numero_de_botones)) ? "hidden" : ""}`} onClick={() => handlePageClick(pageNumber)}>
                        {pageNumber}
                    </div>);
        })}
